...
// Äpi avalik aadress kirjade linkide jaoks. Vercelis töötades
// ignoreerime kogemata sinna jäänud localhost väärtust ja võtame
// Verceli enda antud toodangu-aadressi — nii ei saa loobumislink
// kunagi localhost'i näidata.
export function publicAppUrl(fallbackOrigin) {
  let url = process.env.NEXT_PUBLIC_APP_URL || '';
  if (process.env.VERCEL && url.includes('localhost')) url = '';
  if (!url && process.env.VERCEL_PROJECT_PRODUCTION_URL) {
    url = 'https://' + process.env.VERCEL_PROJECT_PRODUCTION_URL;
  }
  return url || fallbackOrigin || 'http://localhost:3000';
}
...
export function signUnsubscribe(userId, eventId, expiresMs) {
  const payload = `${userId}.${eventId}.${expiresMs}`;
...
export function verifyUnsubscribe(token) {
  ...
  const [userId, eventId, expStr, sig] = parts;
  if (!/^[0-9a-f-]{30,40}$/i.test(userId)) return null;
  if (!/^[0-9a-f-]{30,40}$/i.test(eventId)) return null;
  const exp = Number(expStr);
  if (!Number.isFinite(exp) || Date.now() > exp) return null; // aegunud
  const expected = createHmac('sha256', secret())
    .update(`${userId}.${eventId}.${expStr}`).digest('base64url');
...
export async function sendDailyTo(supabase, user, event, day, appUrl, options = {}) {
  ...
  const { data: stages } = await supabase.from('stages')
    .select('id, name_et, name_en').eq('event_id', event.id);
  ...
  const loobuLink = `${appUrl}/api/loobu?token=${signUnsubscribe(user.id, event.id, expires)}`;
