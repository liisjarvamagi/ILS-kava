...
// ei koormataks festivali tipptunnil. Alates platvormi tükist B
// on kõik sündmusepõhine: iga leht ütleb, MILLISE sündmuse kava
// ta tahab, ja turvareeglid näitavad ainult elusat sündmust.
import { getSupabase, supabaseAvailable } from './supabase';

// Kas kavaandmete allikas on üldse seadistatud? (Kui ei, näitavad
// lehed sõbralikku teadet; kui jah, aga sündmust pole, tuleb 404.)
export function scheduleAvailable() {
  return supabaseAvailable();
}

// Avalike sündmuste loend avastamisvaate jaoks
export async function loadEvents() {
  if (!supabaseAvailable()) return null;
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('events')
    .select('id, slug, name, starts_on, ends_on, cover_image_url')
    .order('starts_on');
  if (error) { console.error('Sündmuste laadimine ebaõnnestus', error); return null; }
  return data;
}

export async function loadSchedule(eventSlug) {
  if (!supabaseAvailable() || !isValidSlug(eventSlug)) return null;
  const supabase = getSupabase();

  // Sündmus slugi järgi. Turvareeglid näitavad anonüümsele ainult
  // elusat (avalik + aktiivne) sündmust — muidu tuleb null → 404.
  const { data: event, error: evErr } = await supabase
    .from('events')
    .select('id, slug, name, starts_on, ends_on, cover_image_url, tickets_url, rules_url')
    .eq('slug', eventSlug)
    .maybeSingle();
  if (evErr || !event) return null;

  const [stagesRes, perfRes, infoRes, mapsRes] = await Promise.all([
    supabase.from('stages')
      .select('id, slug, name_et, name_en, descr_et, descr_en, color, sort_order, lat, lng, map_x, map_y, map_id')
    ...
    supabase.from('event_info')
      ...
      .eq('is_active', true)
      .order('sort_order'),
    supabase.from('event_maps')
      .select('id, title_et, title_en, image_url, sort_order')
      .eq('event_id', event.id)
      .order('sort_order')
  ]);
  ...
  // Oluline info ja kaardid on valikulised
  const info = infoRes.error ? [] : infoRes.data;
  const maps = mapsRes.error ? [] : mapsRes.data;
  return { event, stages: stagesRes.data, performances: perfRes.data, info, maps };
}
